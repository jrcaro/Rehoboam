...
3. Upgrade pip and install all the libraries.
   ...
   ``` 
4. Clone the TensorFlow Models and COCO API repositories.
   ```
   git clone https://github.com/tensorflow/models.git
   git clone https://github.com/cocodataset/cocoapi.git
   ```
5. Download the latest ```protoc-*-*.zip``` release and extract the contents in a directory ```<PATH_TO_PB>``` of your choice.
6. Add ```<PATH_TO_PB>``` to your system Path adding the following lines to ```~/.bashrc```.
   ```
   export PATH=<PATH_TO_PB>${PATH:+:${PATH}}
   source ~/.bashrc
   ```
7. In a new Terminal, navigate to ```models/research/``` and run:
   ```
   protoc object_detection/protos/*.proto --python_out=.
   ```
8. Change to ```cocoapi/PythonAPI``` and run:
   ```
   make
